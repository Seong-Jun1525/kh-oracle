...
* DQL (Data Query Language) - 데이터 검색 언어
  ...
  - 표현법 및 실행 순서 표시
		...
		SELECT 컬럼명 또는 연산식 또는 함수식 또는 *(전체컬럼)
		FROM 테이블명
		WHERE 조건식
		ORDER BY 정렬 기준이 되는 컬럼...
		
		ORDER BY 절의 정렬 기본값은 오름차순, NULLS LAST
									내림차순, NULLS FIRST
...
--SELECT EMP_NAME, CONCAT(SUBSTR(EMP_NO,1, 8), '******') AS "주민 번호", EMAIL, PHONE
SELECT EMP_NAME, RPAD(SUBSTR(EMP_NO,1, 8), 14, '*') AS "주민 번호", EMAIL, PHONE
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, HIRE_DATE, SUBSTR(EMP_NO, 1, 6) AS "생년월일"
FROM EMPLOYEE
