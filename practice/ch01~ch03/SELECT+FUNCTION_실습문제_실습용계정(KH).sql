...
SELECT EMP_NAME, SALARY*12, (SALARY + (BONUS*SALARY))*12, (SALARY + (BONUS*SALARY))*12-(SALARY*12*0.03)
FROM EMPLOYEE;
...
SELECT EMP_NAME, SALARY, (SALARY + (SALARY*BONUS))*12, HIRE_DATE
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D9', 'D5') AND HIRE_DATE < TO_CHAR('020101');

-- 11. EMPLOYEE테이블에 고용일이 90/01/01 ~ 01/01/01인 사원의 전체 내용을 조회
SELECT *
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN TO_DATE('900101') AND TO_DATE('010101');
...
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE '____#_%' ESCAPE '#' AND DEPT_CODE IN ('D9', 'D6') AND HIRE_DATE BETWEEN TO_DATE('90/01/01') AND TO_DATE('00/12/01') AND SALARY >= 2700000;
...
SELECT EMP_NAME, ROUND(ABS(HIRE_DATE - SYSDATE)) AS "근무일수1", ROUND(SYSDATE - HIRE_DATE) AS "근무일수"
FROM EMPLOYEE;
...
--    (단, 부서코드 오름차순으로 정렬)
SELECT EMP_ID, EMP_NAME, DEPT_CODE, CASE 
...
--     주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6) AS "주민번호 앞자리", SUBSTR(EMP_NO, 8) AS "주민번호 뒷자리",
TO_NUMBER(SUBSTR(EMP_NO, 1, 6)) + TO_NUMBER(SUBSTR(EMP_NO, 8)) AS " 주민번호 앞자리와 뒷자리의 합"
FROM EMPLOYEE
WHERE EMP_ID = 201;
